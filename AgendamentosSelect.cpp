...
BOOL RegisterAgendamentosSelectClass(HINSTANCE hInstance)
{
    ...
    wcex.lpfnWndProc = WndProcAgendamentosSelect;
...
LRESULT CALLBACK WndProcAgendamentosSelect(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        AgendamentosSelect_g_wasInactive = false;
        AgendamentosSelect_g_hWndMain = hWnd;
    ...
    case WM_COMMAND:
    {
        ...
        else if (wmId == AgendamentosSelect_OFFSET) {

            AgendamentosSelect_mudarPagina(id);
            AgendamentosSelect_RecarregarDadosTabela(hWnd);
        }
        break;
    }
    case WM_PAINT:
    {
        ...
        //Header Table
        AgendamentosSelect_createHeaderTable(hWnd, hdc);
    ...
    case WM_ACTIVATE:
    {
        if (LOWORD(wParam) == WA_INACTIVE) {
            AgendamentosSelect_g_wasInactive = true; // Marca como inativa quando perde foco para outra janela
        }
    ...
    case WM_KILLFOCUS:
    {
        AgendamentosSelect_g_wasInactive = true; // Marca como inativa quando perde foco para outro controle
        return 0;
    }
    break;
    ...
    case WM_DRAWITEM:
    {
        ...
        if (pDraw->CtlID == AgendamentosSelect_OFFSET) {
