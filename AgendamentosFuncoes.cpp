...
int AgendamentosSelect_g_scrollY;      // Posição vertical do scroll
int AgendamentosSelect_g_contentWidth;   // Largura total do conteúdo
int AgendamentosSelect_g_contentHeight;   // Altura total do conteúdo
int AgendamentosSelect_g_clientWidth;       // Largura da área cliente
int AgendamentosSelect_g_clientHeight;      // Altura da área cliente
int AgendamentosSelect_rowsNumber;
int AgendamentosSelect_limitTableRow = 20;
int AgendamentosSelect_offsetTableRow = 1;
int AgendamentosSelect_numeroBtn;
int AgendamentosSelect_idNumeroUltimo = 1;
int AgendamentosSelect_rowsNumberSemCabecalho = 0;
...
std::vector<HWND> AgendamentosSelect_g_editControls; // Array global para armazenar handles dos controles de edição
std::vector<HWND> AgendamentosSelect_g_editControlsFilters;
std::vector<HWND> AgendamentosSelect_g_editControlsOrder;
std::vector<HWND> AgendamentosSelect_g_editControlsLimit;
std::vector<HWND> AgendamentosSelect_g_editControlsOffsetLimit;
std::vector<std::vector<std::wstring>> AgendamentosSelect_g_tableData;
...
std::vector<std::wstring> AgendamentosSelect_dados(AgendamentosSelect_numberRowsTable);
std::wstring AgendamentosSelect_dataAte;
std::wstring AgendamentosSelect_dataRegistroAte;

std::vector<HWND> AgendamentosSelect_g_buttons;

// Substitua o macro por constexpr conforme sugerido pelo VCR101
constexpr int AgendamentosSelect_MAX_LOADSTRING = 100;

int AgendamentosSelect_windowsNumber = 1;

// Variáveis Globais:
HWND AgendamentosSelect_g_hButton = NULL;
HBRUSH AgendamentosSelect_hBrushTransparent = (HBRUSH)GetStockObject(HOLLOW_BRUSH);

void AgendamentosSelect_DestroyAllControls()
{
    // Itera sobre todos os HWNDs no contêiner
    for (HWND hWndControl : AgendamentosSelect_g_editControlsOrder)
    {
    ...
    AgendamentosSelect_g_editControlsOrder.clear();
...
void AgendamentosSelect_mudarPagina(int id) {
    ...
    int newPageId = AgendamentosSelect_idNumeroUltimo;
    ...
    if (id == 0) {
        ...
        if (AgendamentosSelect_idNumeroUltimo > 1) {
            newPageId = AgendamentosSelect_idNumeroUltimo - 1;
        }
    }
    // ----------------------------------------------------------------------
    // 2. Lógica do Botão 'Próxima Página' (id == numeroBtn + 1)
    // ----------------------------------------------------------------------
    else if (id == AgendamentosSelect_numeroBtn + 1) {
        // Verifica se não estamos na última página
        if (AgendamentosSelect_idNumeroUltimo < AgendamentosSelect_numeroBtn) {
            newPageId = AgendamentosSelect_idNumeroUltimo + 1;
        }
    }
    // ----------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------
        ...
        if (id >= 1 && id <= AgendamentosSelect_numeroBtn) {
            newPageId = id;
        }
        else {
    ...
    if (newPageId != AgendamentosSelect_idNumeroUltimo) {
        AgendamentosSelect_idNumeroUltimo = newPageId;

        // O offset é sempre o Tamanho da Página * (Página - 1)
        AgendamentosSelect_offsetTableRow = AgendamentosSelect_limitTableRow * (AgendamentosSelect_idNumeroUltimo - 1) + 1;
...
void AgendamentosSelect_AtualizarPosicoesOffset(HWND hWnd) {
    ...
    int numButtons = (int)AgendamentosSelect_g_editControlsOffsetLimit.size();
    ...
    for (int i = 0; i < numButtons; i++) {
        ...
        SetWindowPos(
            AgendamentosSelect_g_editControlsOffsetLimit[i],
            NULL,
            xPos,
            yPos,
            buttonWidth,
...
void AgendamentosSelect_DestroyAllOffsetButtons() {
    // 1. Itera sobre todos os handles (HWND) no vetor
    for (HWND hBtn : AgendamentosSelect_g_editControlsOffsetLimit) {
    ...
    AgendamentosSelect_g_editControlsOffsetLimit.clear();
}

void AgendamentosSelect_createBtnPageLimit(HWND hWnd) {
    AgendamentosSelect_DestroyAllOffsetButtons();

    if (AgendamentosSelect_idNumeroUltimo != 1 && AgendamentosSelect_offsetTableRow > AgendamentosSelect_rowsNumberSemCabecalho) {
        AgendamentosSelect_mudarPagina(AgendamentosSelect_idNumeroUltimo - 1);
    }
    ...
    AgendamentosSelect_numeroBtn = static_cast<int>(std::ceil(
        static_cast<double>(AgendamentosSelect_rowsNumberSemCabecalho) / AgendamentosSelect_limitTableRow
    ));

    // Se não houver páginas para exibir (tabela vazia), encerra
    if (AgendamentosSelect_numeroBtn < 1) {
        AgendamentosSelect_AtualizarPosicoesOffset(hWnd);
        return;
    }

    // 2. LÓGICA DE PAGINAÇÃO DINÂMICA (Máximo 10 Botões)
    const int AgendamentosSelect_MAX_PAGES_DISPLAY = 10;

    // Garantir que a página atual (idNumeroUltimo) esteja no intervalo válido
    if (AgendamentosSelect_idNumeroUltimo < 1) AgendamentosSelect_idNumeroUltimo = 1;
    if (AgendamentosSelect_idNumeroUltimo > AgendamentosSelect_numeroBtn) AgendamentosSelect_idNumeroUltimo = AgendamentosSelect_numeroBtn;

    int start_page, end_page;

    // A. Se o total de páginas for menor ou igual ao limite (10)
    if (AgendamentosSelect_numeroBtn <= AgendamentosSelect_MAX_PAGES_DISPLAY) {
        start_page = 1;
        end_page = AgendamentosSelect_numeroBtn;
    }
    // B. Se o total de páginas for maior que 10
    else {
        // Tentativa de centralizar: 5 antes, 4 depois da página atual
        start_page = AgendamentosSelect_idNumeroUltimo - 5;
        end_page = AgendamentosSelect_idNumeroUltimo + 4;

        // B1. Tratamento da borda inicial (garantir que não comece antes de 1)
        if (start_page < 1) {
            start_page = 1;
            end_page = AgendamentosSelect_MAX_PAGES_DISPLAY;
        }

        // B2. Tratamento da borda final (garantir que não ultrapasse numeroBtn)
        else if (end_page > AgendamentosSelect_numeroBtn) {
            end_page = AgendamentosSelect_numeroBtn;
            // Ajusta o início para mostrar os últimos 10 botões
            start_page = AgendamentosSelect_numeroBtn - AgendamentosSelect_MAX_PAGES_DISPLAY + 1;
        }
    }
    ...
    HWND hBotao = CreateWindowEx(
        ...
        hWnd, (HMENU)AgendamentosSelect_OFFSET,
        (HINSTANCE)GetWindowLongPtr(hWnd, GWLP_HINSTANCE), NULL
    );
    SetWindowLongPtr(hBotao, GWLP_USERDATA, 0); // ID 0 para 'Voltar'
    AgendamentosSelect_g_editControlsOffsetLimit.push_back(hBotao);
    ...
    int forward_id = AgendamentosSelect_numeroBtn + 1;
...
void AgendamentosSelect_handleLimitChange(HWND hComboBox) {
    ...
    if (selected_index != CB_ERR) {
        ...
        AgendamentosSelect_limitTableRow = _wtoi(selected_text.c_str());
...
void AgendamentosSelect_AtualizarPosicoesLimit(HWND hWnd) {
    ...
    if (AgendamentosSelect_g_editControlsLimit.empty() || !IsWindow(AgendamentosSelect_g_editControlsLimit[0])) {
        return;
    }

    // Obter o HWND do ComboBox
    HWND hComboBox = AgendamentosSelect_g_editControlsLimit[0];
    ...
    int startYFull = 80 - AgendamentosSelect_g_scrollY;  // Posição Y com scroll
    int xPos;
    int yPos;
...
void AgendamentosSelect_createInputLimit(HWND hWnd) {
    ...
    HWND hComboBox = CreateWindowEx(
        ...
        hWnd,                        // Janela pai
        (HMENU)(AgendamentosSelect_LIMITAR),                        // ID único do controle (para o WM_COMMAND)
    ...
    if (AgendamentosSelect_limitTableRow == 10) {
        SendMessageW(hComboBox, CB_SETCURSEL, 0, 0);
    }
    else if (AgendamentosSelect_limitTableRow == 20) {
        SendMessageW(hComboBox, CB_SETCURSEL, 1, 0);
    }
    else if (AgendamentosSelect_limitTableRow == 30) {
        SendMessageW(hComboBox, CB_SETCURSEL, 2, 0);
    }
    else if (AgendamentosSelect_limitTableRow == 40) {
        SendMessageW(hComboBox, CB_SETCURSEL, 3, 0);
    }
    else if (AgendamentosSelect_limitTableRow == 50) {
        SendMessageW(hComboBox, CB_SETCURSEL, 4, 0);
    }
    else if (AgendamentosSelect_limitTableRow == 75) {
        SendMessageW(hComboBox, CB_SETCURSEL, 5, 0);
    }
    else if (AgendamentosSelect_limitTableRow == 100) {
        SendMessageW(hComboBox, CB_SETCURSEL, 6, 0);
    }

    AgendamentosSelect_g_editControlsLimit.push_back(hComboBox);
}
...
void AgendamentosSelect_AtualizarPosicoesOrder(HWND hWnd) {
    ...
    for (int col = 0; col < 7; col++) {
        ...
        SetWindowPos(AgendamentosSelect_g_editControlsOrder[col], NULL, xPos - 10, yPos, 0, 0,
...
void AgendamentosSelect_ordenarMudarIcone(HWND hWnd) {
    ...
    HWND hButtonOrdenar = GetDlgItem(hWnd, AgendamentosSelect_ORDENAR);

    while (hButtonOrdenar) {
        // Verificar se é um botão de ordenar
        if (GetDlgCtrlID(hButtonOrdenar) >= AgendamentosSelect_ORDENAR &&
            GetDlgCtrlID(hButtonOrdenar) <= AgendamentosSelect_ORDENAR + 7) {

            // Verificar se é o botão da coluna desejada
            LONG_PTR coluna = GetWindowLongPtr(hButtonOrdenar, GWLP_USERDATA);
            if (coluna == 0 && AgendamentosSelect_orderAscDesc == "ASC") {
                // Encontrou o botão certo - mudar ícone
                // Mudar para o novo ícone
                AgendamentosSelect_MudarIconeDoBotao(hButtonOrdenar, IDB_SETAS_BAIXO);
            }
            else if (coluna == 0 && AgendamentosSelect_orderAscDesc == "DESC") {
                AgendamentosSelect_MudarIconeDoBotao(hButtonOrdenar, IDB_SETAS_CIMA);
            }
            else {
                AgendamentosSelect_MudarIconeDoBotao(hButtonOrdenar, IDB_SETAS);
            }
        }
        hButtonOrdenar = GetWindow(hButtonOrdenar, GW_HWNDNEXT);
    }

    for (size_t i = 0; i < AgendamentosSelect_g_editControlsOrder.size(); i++) {
        if (i == AgendamentosSelect_idBtnGlobal && AgendamentosSelect_orderAscDesc == "ASC") {
            HWND hButton = AgendamentosSelect_g_editControlsOrder[i];
            AgendamentosSelect_MudarIconeDoBotao(hButton, IDB_SETAS_CIMA);
        }
        else if (i == AgendamentosSelect_idBtnGlobal && AgendamentosSelect_orderAscDesc == "DESC") {
            HWND hButton = AgendamentosSelect_g_editControlsOrder[i];
            AgendamentosSelect_MudarIconeDoBotao(hButton, IDB_SETAS_BAIXO);
        }
        else {
            HWND hButton = AgendamentosSelect_g_editControlsOrder[i];
            AgendamentosSelect_MudarIconeDoBotao(hButton, IDB_SETAS);
        }
    }
}

void AgendamentosSelect_ordenarDefinicoesValores(HWND hWnd) {
    if (AgendamentosSelect_btnClicado == L"ORDENAR") {
        std::string AgendamentosSelect_oldOrderColumn = AgendamentosSelect_orderColumn;
        ...
        if (AgendamentosSelect_orderAscDesc == "DESC" && AgendamentosSelect_orderColumn == AgendamentosSelect_oldOrderColumn) {
            AgendamentosSelect_orderAscDesc = "ASC";
        }
        else if (AgendamentosSelect_orderAscDesc == "ASC" && AgendamentosSelect_orderColumn == AgendamentosSelect_oldOrderColumn) {
            AgendamentosSelect_orderAscDesc = "DESC";
        }
        else if (AgendamentosSelect_orderColumn != AgendamentosSelect_oldOrderColumn) {
            if (AgendamentosSelect_idBtnGlobal == 0) {
                AgendamentosSelect_orderAscDesc = "DESC";
            }
            else {
                AgendamentosSelect_orderAscDesc = "ASC";
        ...
        AgendamentosSelect_btnClicado = L"";
    }
}
...
void AgendamentosSelect_createOrderBtn(HWND hWnd) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        HWND hButton = CreateWindowW(
            ...
            hWnd,
            (HMENU)(AgendamentosSelect_ORDENAR), // Seu ID de controle (555 é um valor válido)
            (HINSTANCE)GetWindowLongPtr(hWnd, GWLP_HINSTANCE),
            NULL
        );
        ...
        AgendamentosSelect_g_editControlsOrder.push_back(hButton);
    }
}
...
std::tuple<AgendamentosSelect_Data, bool> AgendamentosSelect_parseData(const std::wstring& dataStr) {
    AgendamentosSelect_Data data = { 0, 0, 0 };
    bool sucesso = false;
 ...
 */
bool AgendamentosSelect_estaEntreDatas(const std::wstring& dataIntervalo1,
    const std::wstring& dataIntervalo2,
    const std::wstring& dataParaTestar) {

    AgendamentosSelect_Data dataA;
    bool sucessoA;
    std::tie(dataA, sucessoA) = AgendamentosSelect_parseData(dataIntervalo1);

    AgendamentosSelect_Data dataB;
    bool sucessoB;
    std::tie(dataB, sucessoB) = AgendamentosSelect_parseData(dataIntervalo2);

    AgendamentosSelect_Data dataTeste;
    bool sucessoTeste;
    std::tie(dataTeste, sucessoTeste) = AgendamentosSelect_parseData(dataParaTestar);
    ...
    if (!dataIntervalo1.empty() && !dataIntervalo2.empty()) {
        ...
        const AgendamentosSelect_Data& dataInicio = dataA;
        const AgendamentosSelect_Data& dataFim = dataB;
...
void AgendamentosSelect_AtualizarPosicoesInputs(HWND hWnd) {
    ...
    int cellHeight = 32;
    int startXFull = 22 - AgendamentosSelect_g_scrollX;
    int startYFull = 80 - AgendamentosSelect_g_scrollY;
    int row = 1;
    int xPos;
    int yPos;
    ...
    for (int col = 20; col < AgendamentosSelect_g_editControlsFilters.size() - 1; col++) {
        yPos = startYFull + row * cellHeight + 7;
        xPos = startXFull + (col - colNumber) * cellWidthFull + 10;
    ...
    SetWindowPos(AgendamentosSelect_g_editControlsFilters[AgendamentosSelect_numberRowsTable], NULL, xPos, yPos, 70, 30,
        SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOSIZE | SWP_HIDEWINDOW);
    SetWindowPos(AgendamentosSelect_g_editControlsFilters[AgendamentosSelect_numberRowsTable], NULL, xPos, yPos, 70, 30,
        SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOSIZE | SWP_SHOWWINDOW);
}
...
void AgendamentosSelect_criarInputsFilters(HWND hWnd) {
    ...
    // Configurar a linha do header
    int startYFull = 80 - AgendamentosSelect_g_scrollY;  // Posição Y com scroll
    int startXFull = 22 - AgendamentosSelect_g_scrollX;  // Posição X com scroll
    ...
    HWND hButton = CreateWindowW(
        ...
        xPos, yPos, 70, 30,
        hWnd, (HMENU)(AgendamentosSelect_FILTRAR),
        (HINSTANCE)GetWindowLongPtr(hWnd, GWLP_HINSTANCE), NULL
    );
    if (hButton) {
        //SetWindowLongPtr(hButton, GWLP_USERDATA, controlID + 1);
        AgendamentosSelect_g_editControlsFilters.push_back(hButton);
    }
}

void AgendamentosSelect_criarHeaderLineFilter(HDC hdc, HWND hWnd, int startYFull, int startXFull, int colNumber, int colFinalNumber) {
    // Obter dimensões da janela
    RECT rect;
    GetClientRect(hWnd, &rect);
    int width = (rect.right - rect.left) - 44;
    ...
    for (size_t row = 0; row < 1; row++) {
        ...
        for (size_t col = colNumber; col < colFinalNumber; col++) {
            ...
            std::wstring displayText = AgendamentosSelect_g_tableDataFull[row][col];

            // Traduzir cabeçalhos se necessário
            if (row == 0) {
                displayText = AgendamentosSelect_arrumarNomesColunas(displayText);
            }
...
void AgendamentosSelect_createHeaderTable(HWND hWnd, HDC hdc) {
    AgendamentosSelect_selectHeaderDB();
    ...
    hCurrentBrush = hBrushHeader;
    AgendamentosSelect_fonte(L"Header", RGB(255, 255, 255), hdc);
    ...
    int counter = 0;
    for (size_t col = 0; col < AgendamentosSelect_g_tableDataFull[0].size(); col++) {
        std::wstring displayText = AgendamentosSelect_g_tableDataFull[0][col];
...
void AgendamentosSelect_selectHeaderDB() {
    ...
    rc = sqlite3_exec(
        db,
        sqlPragma,
        AgendamentosSelect_pragma_callback, // Nosso callback personalizado
        &AgendamentosSelect_g_tableDataFull, // Passamos o vetor global para o callback
        &errMsg
    );
...
void AgendamentosSelect_updateWindow(LPCWSTR className) {
    ...
    if (hSelectWnd != NULL)
    {
        AgendamentosSelect_selectDB();
        AgendamentosSelect_RecarregarDadosTabela(hSelectWnd);
    }
}
...
void AgendamentosSelect_AtualizarPosicoesBotoes(HWND hWnd)
{
    ...
    for (size_t i = 0; i < AgendamentosSelect_g_buttons.size(); i++) {
        size_t row = (i / 3) + 1;
        int buttonType = i % 3;

        int yPos = startY + row * cellHeight + 2 - AgendamentosSelect_g_scrollY;
        ...
        if (isVisible) {
            SetWindowPos(AgendamentosSelect_g_buttons[i], NULL, xPos, yPos, 0, 0,
                SWP_NOSIZE | SWP_NOZORDER | SWP_SHOWWINDOW);
        }
        else {
...
void AgendamentosSelect_selectDB() {
    ...
    if (rc == SQLITE_OK) {
        ...
        if (AgendamentosSelect_orderColumn == "Appointment_Hour") {
            ...
            sqlSelect = "SELECT * FROM Pets ORDER BY " + hourSorting + " " + AgendamentosSelect_orderAscDesc;
        }
            ...
            sqlSelect = "SELECT * FROM Pets ORDER BY " + dataSorting + " " + AgendamentosSelect_orderAscDesc;

        }
        else {
            //const char* sqlSelect = "SELECT ID, Nome_do_Pet, Nome_do_Tutor, Banho, Tosa, Appointment_Date, Appointment_Hour FROM Pets;";
            sqlSelect = "SELECT * FROM Pets ORDER BY " + AgendamentosSelect_orderColumn + " COLLATE NOCASE " + AgendamentosSelect_orderAscDesc;
        }
        ...
        rc = sqlite3_exec(db, sqlSelect.c_str(), AgendamentosSelect_sqlite_callback, &AgendamentosSelect_g_tableData, &errMsg);
        if (rc != SQLITE_OK) {
            if (errMsg) {
                ...
                AgendamentosSelect_g_tableData.push_back({ L"Erro", wErrMsg });
            }
            else {
                AgendamentosSelect_g_tableData.push_back({ L"Erro", L"Desconhecido" });
            }
            if (errMsg) sqlite3_free(errMsg);
        }
        sqlite3_close(db);
        AgendamentosSelect_naoDesenhar.resize(AgendamentosSelect_g_tableData.size());
    }
    else {
        AgendamentosSelect_g_tableData.push_back({ L"Erro", L"Não foi possível abrir o banco" });
    }
}
...
void AgendamentosSelect_verificarFiltro(const std::vector<std::wstring>& dados, std::vector<int>& naoDesenharIntern) {
    // 1. LIMPAR DADOS ANTIGOS ANTES DE CADA CONSULTA
    AgendamentosSelect_g_tableDataRowsNumber.clear();

    // CORREÇÃO: Verificar se g_tableData não está vazio antes de redimensionar
    if (!AgendamentosSelect_g_tableData.empty()) {
        AgendamentosSelect_g_tableDataRowsNumber.resize(AgendamentosSelect_g_tableData.size());
        for (size_t row = 0; row < AgendamentosSelect_g_tableData.size(); ++row) {
            AgendamentosSelect_g_tableDataRowsNumber[row].resize(AgendamentosSelect_g_tableData[row].size());
        }
    }
    ...
    if (naoDesenharIntern.size() != AgendamentosSelect_g_tableData.size()) {
        naoDesenharIntern.resize(AgendamentosSelect_g_tableData.size(), 0);
    }

    int column;
    int numeroColIteracoes = AgendamentosSelect_numberRowsTable;
    for (size_t row = 0; row < AgendamentosSelect_g_tableData.size(); row++) {
        ...
        for (size_t col = 0; col < numeroColIteracoes; col++) {
            ...
            dadoTable = AgendamentosSelect_g_tableData[row][column];
            ...
            if (!dados[col].empty() && (col == 0 || col == 6 || col == 7 || col == 9 || col == 12 || col == 13 || col == 15 || col == 16)) {
                ...
                if (col == 21) {
                    dadoTable = AgendamentosSelect_g_tableData[row][col - 1];

                }

                bool estaEntre = AgendamentosSelect_estaEntreDatas(dados[20], dados[21], dadoTable);
                if (!estaEntre) {
                ...
                // CORREÇÃO: Verificar se a coluna existe na linha atual
                if (col == 24) {
                    dadoTable = AgendamentosSelect_g_tableData[row][col - 2];

                }

                bool estaEntre = AgendamentosSelect_estaEntreDatas(dados[23], dados[24], dadoTable);
                if (!estaEntre) {
                ...
                if (AgendamentosSelect_toLower(dadoTable).find(AgendamentosSelect_toLower(dados[col])) == std::wstring::npos) {
                    naoDesenharIntern[row] = 1;
                    break;
                }
            }
    ...
    AgendamentosSelect_rowsNumber = 0;
    int AgendamentosSelect_rowCount = 0;
    ...
    if (!AgendamentosSelect_g_tableData.empty()) {
        AgendamentosSelect_g_tableDataRowsNumber.reserve(AgendamentosSelect_g_tableData.size());
    }

    // 3. Iteração, Filtro e Construção do Novo Vetor
    for (size_t row = 0; row < AgendamentosSelect_g_tableData.size(); row++) { // Começar da linha 1 (pular cabeçalho)
        ...
        if (row < naoDesenharIntern.size() && naoDesenharIntern[row] != 1) {
            ...
            for (size_t col = 0; col < AgendamentosSelect_g_tableData[row].size(); col++) {
                newRow.push_back(AgendamentosSelect_g_tableData[row][col]);
            }

            // C. Adiciona a linha válida (newRow) ao vetor de destino
            AgendamentosSelect_g_tableDataRowsNumber.push_back(newRow);

            // D. Atualiza o contador de linhas válidas
            AgendamentosSelect_rowCount++;
            AgendamentosSelect_rowsNumber++;
        }
    ...
    if (AgendamentosSelect_rowsNumber == 0 && !AgendamentosSelect_g_tableDataRowsNumber.empty()) {
        AgendamentosSelect_rowsNumber = static_cast<int>(AgendamentosSelect_g_tableDataRowsNumber.size());
    }
    if (AgendamentosSelect_rowsNumber == 0 && !AgendamentosSelect_g_tableData.empty()) {
        AgendamentosSelect_rowsNumber = 1; // Pelo menos o cabeçalho
    }
    ...
    if (!AgendamentosSelect_g_tableDataRowsNumber.empty()) {
        AgendamentosSelect_g_tableData = std::move(AgendamentosSelect_g_tableDataRowsNumber);
    }
    else {
        ...
        if (!AgendamentosSelect_g_tableData.empty()) {
            ...
            temp.push_back(AgendamentosSelect_g_tableData[0]);
            AgendamentosSelect_g_tableData = std::move(temp);
            AgendamentosSelect_rowsNumber = 1;
        }
    }

    // CORREÇÃO: Garantir que naoDesenhar tenha o tamanho correto
    if (AgendamentosSelect_naoDesenhar.size() != AgendamentosSelect_g_tableData.size()) {
        AgendamentosSelect_naoDesenhar.resize(AgendamentosSelect_g_tableData.size(), 0);
    }

    // CORREÇÃO: Copiar apenas os valores relevantes
    for (size_t i = 1; i < naoDesenharIntern.size() && i < AgendamentosSelect_naoDesenhar.size(); i++) {
        AgendamentosSelect_naoDesenhar[i] = naoDesenharIntern[i];
    }

    AgendamentosSelect_rowsNumberSemCabecalho = AgendamentosSelect_rowsNumber - 1;
}

void AgendamentosSelect_CriarBotoesTabela(HWND hWnd)
{
    // Limpar botões existentes
    for (HWND hButton : AgendamentosSelect_g_buttons) {
        DestroyWindow(hButton);
    }
    AgendamentosSelect_g_buttons.clear();

    if (AgendamentosSelect_g_tableData.size() <= 1) return;

    RECT rect;
    GetClientRect(hWnd, &rect);
    int width = (rect.right - rect.left) - 44;
    int numColumns = AgendamentosSelect_g_tableData.empty() ? 0 : AgendamentosSelect_g_tableData[0].size() + 3;
    int cellWidth = width / (numColumns > 0 ? numColumns : 1);
    int startX = 22;
    int startY = 80;
    int cellHeight = 32;
    ...
    if (limit < AgendamentosSelect_rowsNumberSemCabecalho) {
        ...
        limit = AgendamentosSelect_rowsNumberSemCabecalho + 1;
    }

    int inicio;
    if (AgendamentosSelect_offsetTableRow == 0) {
        inicio = 1;
    }
    else {
        inicio = AgendamentosSelect_offsetTableRow;
    }

    for (size_t row = inicio; row < limit; row++) {
        LONG_PTR recordId = _wtoi(AgendamentosSelect_g_tableData[row][0].c_str());
        ...
        HWND hButton = CreateWindowW(
            ...
            xPos, yPos, 70, 30,
            hWnd, (HMENU)(AgendamentosSelect_CONSULTAR),
            (HINSTANCE)GetWindowLongPtr(hWnd, GWLP_HINSTANCE), NULL
        );
        if (hButton) {
            SetWindowLongPtr(hButton, GWLP_USERDATA, recordId);
            AgendamentosSelect_g_buttons.push_back(hButton);
        }
        ...
        hButton = CreateWindowW(
            ...
            WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
            xPos, yPos, 70, 30,
            hWnd, (HMENU)(AgendamentosSelect_EDITAR),
            (HINSTANCE)GetWindowLongPtr(hWnd, GWLP_HINSTANCE), NULL
        );
        if (hButton) {
            SetWindowLongPtr(hButton, GWLP_USERDATA, recordId);
        ...
        hButton = CreateWindowW(
            ...
            WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
            xPos, yPos, 70, 30,
            hWnd, (HMENU)(AgendamentosSelect_DELETAR),
            (HINSTANCE)GetWindowLongPtr(hWnd, GWLP_HINSTANCE), NULL
        );
        if (hButton) {
            SetWindowLongPtr(hButton, GWLP_USERDATA, recordId);
...
void AgendamentosSelect_ConfigurarScrollBars(HWND hWnd)
{
    ...
    int limit;
    int numeroDeLinhas = AgendamentosSelect_limitTableRow;
...
void AgendamentosSelect_RecarregarDadosTabela(HWND hWnd) {
    ...
    AgendamentosSelect_DestroyControlsFromVector(AgendamentosSelect_g_buttons);
    AgendamentosSelect_DestroyControlsFromVector(AgendamentosSelect_g_editControlsOffsetLimit);
    AgendamentosSelect_DestroyControlsFromVector(AgendamentosSelect_g_editControlsLimit);
    AgendamentosSelect_DestroyControlsFromVector(AgendamentosSelect_g_editControlsOrder);
    AgendamentosSelect_DestroyControlsFromVector(AgendamentosSelect_g_editControlsFilters);

    // Limpeza extra para garantir
    AgendamentosSelect_g_buttons.clear();
    AgendamentosSelect_g_editControlsOffsetLimit.clear();
    AgendamentosSelect_g_editControlsLimit.clear();
    AgendamentosSelect_g_editControlsOrder.clear();
    AgendamentosSelect_g_editControlsFilters.clear();

    // Definir valores para ordenação e imagem
    AgendamentosSelect_ordenarDefinicoesValores(hWnd);

    // Limpar dados antigos
    AgendamentosSelect_selectDB();

    // Criar inputs de limite
    AgendamentosSelect_createInputLimit(hWnd);
    ...
    AgendamentosSelect_SetFilterValues(AgendamentosSelect_dados);

    // Criar inputs de order
    AgendamentosSelect_createOrderBtn(hWnd);

    //Atualizar posição dos botões
    AgendamentosSelect_AtualizarPosicoesBotoes(hWnd);

    //Atualizar inputs
    AgendamentosSelect_AtualizarPosicoesInputs(hWnd);

    // Atualizar offset botões
    AgendamentosSelect_AtualizarPosicoesOffset(hWnd);

    // Atualizar limite de linhas
    AgendamentosSelect_AtualizarPosicoesLimit(hWnd);

    // Atualizar botões order
    AgendamentosSelect_AtualizarPosicoesOrder(hWnd);

    // Mudar ícone do botão de ordenamento
    AgendamentosSelect_ordenarMudarIcone(hWnd);
    ...
    AgendamentosSelect_ConfigurarScrollBars(hWnd);
...
int AgendamentosSelect_sqlite_callback(void* data, int argc, char** argv, char** azColName) {
    ...
    if (table->empty()) {
        ...
        for (int i = 0; i < argc; i++) {
            headers.push_back(azColName[i] ? AgendamentosSelect_utf8_to_wstring(azColName[i]) : L"NULL");
        }
    ...
    for (int i = 0; i < argc; i++) {
        row.push_back(argv[i] ? AgendamentosSelect_utf8_to_wstring(argv[i]) : L"NULL");
    }
    table->push_back(row);
...
void AgendamentosSelect_AtualizarPosicoesControlesAgendamento(HWND hWnd)
{
    ...
    for (size_t i = 0; i < AgendamentosSelect_g_editControls.size(); i++) {
        ...
        if (i == 11) {
            ...
            xPos = startX + cellWidth + 10;
            yPos = startY + colNumber * cellHeight + 3;
            SetWindowPos(AgendamentosSelect_g_editControls[i], NULL, xPos, yPos, 700, 25,
                SWP_NOZORDER | SWP_NOACTIVATE);
            countRow++;
        }
    }
...
void AgendamentosSelect_ConfigurarScrollBarsAgendamento(HWND hWnd)
{
    ...
    int cellHeight = 32;
    AgendamentosSelect_g_contentHeight = 22 * cellHeight + 100; // 19 campos + título + botão + margem
...
std::wstring AgendamentosSelect_treatDataAppointment(std::wstring dado, int number) {
    ...
    AgendamentosSelect_msg = AgendamentosSelect_mensagem.c_str();
...
BOOL AgendamentosSelect_windowsTitle(HDC hdc, int startX, int startY, LPCWSTR tit, int size) {
    AgendamentosSelect_fonte(L"Title", RGB(0, 0, 0), hdc);
...
{
    // Variáveis de scroll
    AgendamentosSelect_g_scrollX = scrollX;      // Posição horizontal do scroll
    AgendamentosSelect_g_scrollY = scrollY;      // Posição vertical do scroll
    AgendamentosSelect_g_contentWidth = contentWidth;   // Largura total do conteúdo
    AgendamentosSelect_g_contentHeight = contentHeight;   // Altura total do conteúdo
    AgendamentosSelect_g_clientWidth = clientWidth;       // Largura da área cliente
    AgendamentosSelect_g_clientHeight = clientHeight;      // Altura da área cliente
...
BOOL AgendamentosSelect_CreateNewWindow(HWND hWndParent, HINSTANCE hInst, LPCWSTR className, LPCWSTR windowTittle)
{
    ...
    if (hSelectWnd != NULL)
    {
        ...
        wc.lpfnWndProc = WndProcAgendamentosSelect;
...
//
//
LRESULT CALLBACK AgendamentosSelect_WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
...
INT_PTR CALLBACK AgendamentosSelect_About(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
