...
fs::path GetDatabasePath()
{
    ...
    // Cria a pasta com permissão total
    std::error_code ec;
    if (!fs::create_directories(dbDir, ec)) {
        if (ec) {
            // Se falhar (ex: sem permissão), usa pasta temporária
...
bool OpenDatabase(sqlite3*& db)
{
    ...
    try {
        ...
        if (!fs::exists(dbPath)) {
            MessageBox(nullptr,
                (L"Arquivo não encontrado após criação:\n" + dbPath.wstring()).c_str(),
                L"Erro", MB_OK | MB_ICONERROR);
            return false;
        }

        // === Usa string() → UTF-8 seguro no Windows 10/11 ===
